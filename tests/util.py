...
def sha1sum(data: bytes) -> str:
  """Perform sha1sum on some data.

    Args:

      data (bytes): Data to sha1 hash.

    Returns:

      str: Hexdigest (sha1sum) of data.
    """
...
def fuse_test_marker():
  ...
  which = subprocess.Popen(['which', 'fusermount'],
...
def wait_for(callable_, timeout=10, interval=0.1):
...
def cleanup(mnt_dir):
  if platform.system() == 'Darwin':
    subprocess.call(['umount', '-l', mnt_dir],
